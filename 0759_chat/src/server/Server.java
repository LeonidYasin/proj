                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            while (true){
                                    ...
                                    System.out.println("Клиент " +currentUser.getUserName()+
                                            " прислал: "+request);
                            ...
                            for (User user: users) { // Перебираем клиентов которые подключенны в настоящий момент
                                try {
                                    user.getOos().writeObject("Пользователь "+currentUser.getUserName()+" покинул чат"); // Рассылает принятое сообщение всем клиентамcur
                            ...
                            try {
                                currentUser.getSocket().close();// Удаление сокета, когда клиент
                                currentUser.getOos().reset();
                                currentUser.getOos().close();
                            } catch (IOException ioException) {
                                ioException.printStackTrace();
                            }
                            sendUserList(); //Отправляем обновление списка пользователей
